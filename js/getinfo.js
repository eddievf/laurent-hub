...
function showforValid(str){
   if (str == "") {
        document.getElementById("valorHint").innerHTML = "";
        return;
    }
    else { 
        if (window.XMLHttpRequest) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("valorHint").innerHTML = this.responseText;
            }
        };
        xmlhttp.open("GET","php/getforvalid.php?dart="+str,true);
        xmlhttp.send();
    }
}
